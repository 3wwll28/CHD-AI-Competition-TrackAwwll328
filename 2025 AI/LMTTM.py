...
class PreProcess3D(nn.Module): 
    ...
    def forward(self, input):
        print(f"      🔧 PreProcess3D输入: {input.shape}")
        
        # 🔧 修复: 适配不同维度的输入
        if input.dim() == 4:
            # 4维输入: [batch, features, tokens, 1] -> 5维: [batch, 1, features, tokens, 1]
            input = input.unsqueeze(1)  # 添加通道维
            print(f"         ✅ 4维转5维: {input.shape}")
        elif input.dim() == 5:
            # 已经是5维，保持不变
            pass
        else:
            raise ValueError(f"不支持的输入维度: {input.dim()}")
        ...
        print(f"      ✅ PreProcess3D输出: {x.shape}")
        return x
...
class PreProcess3DWithBN(nn.Module): 
    ...
    def forward(self, input):
        print(f"      🔧 PreProcess3DWithBN输入: {input.shape}")
        ...
        print(f"      ✅ PreProcess3DWithBN输出: {x.shape}")
        return x
...
class PreProcessResnet18(nn.Module):
    ...
    def forward(self, x):
        print(f"      🔧 PreProcessResnet18输入: {x.shape}")
        
        # 🔧 修复: 适配不同维度的输入
        if x.dim() == 4:
            # 假设输入是 [batch, sequence, tokens, features] -> 需要转换为图像格式
            batch_size, sequence, tokens, features = x.shape
            # 这里需要根据实际情况调整
            x = x.reshape(batch_size * sequence, tokens, features, 1)
            x = x.repeat(1, 1, 1, 3)  # 复制到3通道
            print(f"         ✅ 适配输入形状: {x.shape}")
        ...
        print(f"      ✅ PreProcessResnet18输出: {x.shape}")
        return x
...
class TokenLearnerMHA(nn.Module):
    ...
    def forward(self, input):
        print(f"        🔧 TokenLearnerMHA输入: {input.shape}")
        output = self.attn(self.query, input, input)[0]
        print(f"        ✅ TokenLearnerMHA输出: {output.shape}")
        return output
...
class TokenAddEraseWrite(nn.Module):
    ...
    def forward(self, memory_tokens, control_inputs):
        print(f"        🔧 TokenAddEraseWrite输入: memory_tokens{memory_tokens.shape}, control{control_inputs.shape}")
        ...
        print(f"        ✅ TokenAddEraseWrite输出: {output.shape}")
        return output
...
class TokenTuringMachineUnit(nn.Module):
    ...
    def forward(self, current_memory_block, prev_memory_block, next_memory_block, input_tokens):
        print(f"        🔧 TokenTuringMachineUnit输入:")
        print(f"           current_memory_block: {current_memory_block.shape}")
        print(f"           prev_memory_block: {prev_memory_block.shape}")
        print(f"           next_memory_block: {next_memory_block.shape}")
        ...
        print(f"        🔧 拼接后形状:")
        print(f"           current_all_tokens: {current_all_tokens.shape}")
        print(f"           prev_all_tokens: {prev_all_tokens.shape}")
        print(f"           next_all_tokens: {next_all_tokens.shape}")
        ...
        print(f"        ✅ TokenTuringMachineUnit输出: {memory_output_tokens.shape}")
...
class TokenTuringMachineEncoder(nn.Module):
    ...
    def forward(self, input, memory_tokens):
        print(f"🔧 LMTTM输入形状修复: {input.shape}")
        
        # 🔧 修复: 适配输入形状 - 将4维输入转换为5维
        original_shape = input.shape
        if input.dim() == 4:
            # 4维输入: [batch, sequence, tokens, features] -> 5维: [batch, features, sequence, tokens, 1]
            batch_size, sequence, tokens, features = input.shape
            input = input.permute(0, 3, 1, 2)  # [batch, features, sequence, tokens]
            input = input.unsqueeze(-1)         # [batch, features, sequence, tokens, 1]
            print(f"   ✅ 4维转5维: {original_shape} -> {input.shape}")
        elif input.dim() == 5:
            # 已经是5维，保持不变
            print(f"   ✅ 输入已经是5维: {input.shape}")
        else:
            raise ValueError(f"不支持的输入维度: {input.dim()}")
        ...
        print(f"✅ LMTTM处理完成: 输入 {original_shape} -> 输出 {out.shape}")
